...
const renderLineWithAnsiAndHighlights = (
  ...
  for (const seg of segments) {
    ...
    const overlapping = matchRanges.filter(
      (m) => m.end > segStart && m.start < segEnd
    );
    ...
    for (const m of sorted) {
      ...
      parts.push(
        <mark
          key={parts.length}
          className={isCurrent ? "bg-amber-500 text-slate-900 px-0.5" : "bg-slate-600 px-0.5"}
        >
...
export const OutputPanel = ({
  ...
  const getSourceLineIndex = useCallback(
    (displayIndex: number) =>
      filterLines ? filteredIndices[displayIndex] : displayIndex,
    [filterLines, filteredIndices]
  );

  const scrollToLine = useCallback(
    (lineIndex: number) => {
      const displayIndex = filterLines
        ? filteredIndices.indexOf(lineIndex)
        : lineIndex;
  ...
  useEffect(() => {
    if (
      currentMatchIndex !== lastScrolledMatchIndexRef.current &&
  ...
  const itemContent = (displayIndex: number) => {
    ...
    const isCurrentLine =
      currentMatch != null && currentMatch.lineIndex === sourceLineIndex;
    ...
    return (
      ...
      >
        {renderLineWithAnsiAndHighlights(
          rawLine,
          lineMatches,
          currentMatch,
          isCurrentLine
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0 min-h-0">
      ...
      {displayLength === 0 ? (
        <div className="flex-1 overflow-auto py-3 px-4 font-mono text-[13px] text-slate-500">
          No output yet.
        </div>
      ) : (
