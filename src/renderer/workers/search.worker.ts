...
const runFuzzy = (plainLines: string[], query: string): { matches: Match[]; filteredLineIndices: number[] } => {
    const matches: Match[] = []
    const filteredLineIndices: number[] = []
    ...
    matches.sort((a, b) => (a.lineIndex !== b.lineIndex ? a.lineIndex - b.lineIndex : a.start - b.start))
    return { matches, filteredLineIndices }
}
