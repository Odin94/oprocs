...
export const buttonCls =
    "py-0.5 px-1.5 text-[11px] border border-slate-600 rounded-md bg-slate-800 text-slate-200 cursor-pointer hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"

export const ProcessList = ({ procs, selectedProcId, onSelect, onStart, onStop, onRestart }: ProcessListProps) => (
        ...
        {procs.map((proc) => (
            ...
            >
                <div className="flex items-center gap-2 min-w-0">
                    <span
                        className={`shrink-0 w-2 h-2 rounded-full ${proc.status === "running" ? "bg-green-500" : "bg-slate-500"}`}
                ...
                <div className="flex gap-1.5 pl-4">
                    ...
                    {proc.openUrl ? (
                        <button
                            ...
                            onClick={async (e) => {
                                e.stopPropagation()
                                api?.openExternalLink(proc.openUrl!)
                            }}
                        >
                            Open
                        </button>
