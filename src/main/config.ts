...
const isLikelyWindowsExecutablePath = (value: string): boolean => {
    const trimmed = value.trim()
    const unquoted = /^".*"$/.test(trimmed) ? trimmed.slice(1, -1) : trimmed
    return (
        (/^[a-z]:[\\/]/i.test(unquoted) || unquoted.startsWith("\\\\")) && /\.(?:bat|cmd|com|exe|ps1)$/i.test(unquoted)
    )
}

/**
 * Rewrites shell-like cmd arrays to the local shell for compatibility.
 *
 * `cmd` is formally an argv array, but mprocs configs sometimes use a single
 * shell command string inside the array. On Windows that needs cmd.exe because
 * CreateProcess does not split a command line into an executable and args.
 * Returns `null` when no rewrite is needed.
 */
export const normalizeCmdForPlatform = (cmd: string[], platform: string): string[] | null => {
    if (platform === "win32") {
        if (cmd.length === 1 && /\s/.test(cmd[0]) && !isLikelyWindowsExecutablePath(cmd[0])) {
            return ["cmd", "/c", cmd[0]]
        }
        return null
    }
                ...
                procs[name] = {
                    ...
                    env:
                        proc.env && typeof proc.env === "object"
                            ? (resolveValue(proc.env) as Record<string, string | null>)
                            : undefined,
                    add_path:
                        proc.add_path != null
                            ? Array.isArray(proc.add_path)
                                ? proc.add_path
                                : [proc.add_path]
                            : undefined,
